#!/usr/bin/env python
"""
Token-level optimization experiment for Dyconex sample 236 — smoothmax objective.

Same as run_dyconex_token_level_experiment.py but uses J_sm (smooth-max, beta=50)
instead of J_mean. Both targets A and B, 3 seeds, 1000 iterations.
...
Arguments:
  ...
  --beta             smoothmax temperature parameter (default: 50.0)
...
class TokenLevelRunner:
    def __init__(
        ...
        # For each token position, record var_id and var_name for output
    ...
    # beta controls how closely J_sm approximates the true maximum:
    # higher beta -> sharper approximation but potentially harder to optimize
    SMOOTHMAX_BETA: float = 50.0
    ...
    def score_tensor(self, controls_t: Optional[torch.Tensor], target: str) -> torch.Tensor:
        ...
        vals = pred_y[:, sl, 0].reshape(-1)
        beta = self.SMOOTHMAX_BETA
        # J_sm = (1/beta) * log(sum(exp(beta * y_t))) — smooth approximation of the trajectory peak
        # logsumexp is numerically stable and equivalent to the formula above
        return torch.logsumexp(vals * beta, dim=0) / beta
